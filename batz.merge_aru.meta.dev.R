# =============================================================================
# batz.merge_aru.meta.dev.R
# -----------------------------------------------------------------------------
# Dev script for batz.merge_aru.meta() - tested against real test data
# before being wrapped into the final function (batz.merge_aru.meta.R).
#
...
# batz.merge_aru.meta() to match the project's established convention
...
# batz.merge_aru.meta() now takes dir.load, load.pattern, dir.sub, and
...
# "\\.(csv|xlsx)$" pattern. batz.merge_aru.meta() now takes dir.load,
...
# calling batz.merge_aru.meta(...) - with no assignment needed -
...
res.top <- batz.merge_aru.meta("/home/claude/arumeta_work/top", dir.sub = FALSE)
...
res.all <- batz.merge_aru.meta("/home/claude/arumeta_work/top", dir.sub = TRUE)
...
res.nolog <- batz.merge_aru.meta("/home/claude/arumeta_work/top", dir.sub = TRUE)
cat("has arumeta.mergelog:", !is.null(res.nolog$arumeta.mergelog), "\n")

cat("\n\n=== log.file = TRUE - arumeta.mergelog contents ===\n")
res.log <- batz.merge_aru.meta("/home/claude/arumeta_work/top", dir.sub = TRUE, log.file = TRUE)
...
batz.merge_aru.meta("/home/claude/arumeta_work/top", dir.sub = TRUE, log.file = TRUE)
